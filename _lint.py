...
第三、四项是「长条拖动」的几条线（见文件末尾），也是实测踩出来的：
`_EX_BASE` 里混进 `WS_EX_TRANSPARENT` 就收不到鼠标；把手机制一旦长回来，
两端那两列小圆点就又出现了（用户明确否掉过）；运行期换扩展样式不带
`SWP_FRAMECHANGED` 则改动可能不生效。几种失效都**不影响编译、也不影响截图**，
