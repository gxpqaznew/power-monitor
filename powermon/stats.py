...
class StatsWindow:
    ...
    def create(self) -> bool:
        ...
        # COLOR_BTNFACE 就是 DWM 给对话框的浅灰底，毛玻璃的色调层用它同色，
        # 只是多一层雾 + 模糊；浓度偏高是因为这窗口几乎被原生控件盖满，
        # 能看见的只有四周一圈边，糊太狠反而显脏。
    ...
    def _frost_bg(self, dc, w, h) -> None:
        ...
        if info is None:
            return
        ox, oy, _cw, _ch = info
        frost.blit(dc, "stats", ox, oy, 0, 0, w, h,
    ...
    def _on_message(self, msg, wparam, lparam):
        if msg == WM_ERASEBKGND:
            # 自己接管擦背景：填实色 + 盖毛玻璃。不接管的话 DefWindowProc 只填
            # hbrBackground，那点边就是一块死灰。
            rect = wintypes.RECT()
            user32.GetClientRect(self._hwnd, ctypes.byref(rect))
