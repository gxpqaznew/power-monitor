...
WM_TIMER = 0x0113
WM_COMMAND = 0x0111
# 子控件（Tab / ListView 这类）用 WM_NOTIFY 把「选了别的页」这类事件报给父窗口，
# 载荷是指向 NMHDR 的指针。（wparam 是控件 ID，但**别信它** —— 官方文档说
# 应该用 NMHDR.idFrom，两者在某些控件上并不一致。）
WM_NOTIFY = 0x004E
WM_SIZE = 0x0005
...
WS_CLIPCHILDREN = 0x02000000   # 重绘父窗口时不刷子控件那块，避免闪
WS_THICKFRAME = 0x00040000     # 可拖边框（统计窗口要能拉大看表）
